		...
		<c:forEach  var="aritcle" items="${articlePAPE.content}">
			<tr>
			  <%-- ${aritcle.number}은 Article클래스의 getNumber()메소드를 호출 --%>
				...
				<td width="250"><a href="read.do?no=${aritcle.number}&pageNo=${articlePAPE.currentPage}">${aritcle.title}</a></td>
	...
	<c:if test="${articlePAPE.hasArticles() }">
		<tr>
			<th colspan="4">
				...
				<a href="list.do?pageNo=${articlePAPE.startPage-5}">[이전]</a>
				</c:if>
				...
				<a href="list.do?pageNo=${articlePAPE.startPage+5}">[다음]</a>
				</c:if>
			</th>
		</tr>
	</c:if>
