...
public class FindHandler implements CommandHandler {
	...
	//View지정(p598 16)
	private static final String FORM_VIEW = "/view/member/findForm.jsp";
	private FindService findService = new FindService();
	
	@Override
	public String process(HttpServletRequest request, 
						  HttpServletResponse response) throws Exception {
		System.out.println("FindHandler 진입성공");
		...
		if(request.getMethod().equalsIgnoreCase("GET")) {
			System.out.println("findForm.jsp의 method방식="+request.getMethod());
	...
	private String processSubmit(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			//리턴타입이 Find클래스인경우
			Find find = findService.find(name, email);
			System.out.println("FindHandler클래스의 processSubmit() find ="+find);
			
			
			
			//3.Model
			request.setAttribute("FIND", find); //객체데이터를 모델로
