...
<div class="container">
	...
	<form name="joinFrm" id="joinFrm" onsubmit="return checks()"
		...
		<div class="form-group">
			아이디 : <input type="text" name="id" id="id" value="${param.id}" placeholder="4~12자의 영문 대소문자와 숫자로만 입력" maxlength="12" onkeydown="inputIdChk()"  />
			<c:if test="${errors.id}">아이디를 입력하세요.</c:if>
			<c:if test="${errors.duplicateId}">이미 사용중인 아이디입니다.</c:if>		
		</div>
		<div class="form-group">
			비밀번호 : <input type="password" name="password" id="password" value="${param.password}" placeholder="4~12자의 영문 대소문자와 숫자로만 입력" />
		...
		<div class="form-group">
			이 름 : <input type="text" name="name" id="name" value="${param.name}" placeholder="이름" />
      ...
      <input type="radio" name="gender" value="남자" checked="checked"/>남자
       ...
       <c:if test="${errors.birth}">생일을 입력하세요.</c:if>
		...
		<div class="form-group">
		전화번호 : 
		<select name="phone1" id="phone1" value="${param.phone1}">
       <option value="">선택</option>
     ...
     <input type="text" name="phone2" id="phone2" value="${param.phone2}" size="5" maxlength="4"> - 
     <input type="text" name="phone3" id="phone3" value="${param.phone3}" size="5" maxlength="4">
		 <c:if test="${errors.phone}">번호를 입력하세요.</c:if>
		</div>
		<div class="form-group">
		이메일 :
		<input type="text" name="email" id="email" value="${param.email}" placeholder="abc@doseo.com" maxlength="50">
